...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        temp = np.maximum(zeros,temp)
        temp = temp.dot(W2) + b2.T
        scores = temp
        pass
        ...
        X3 = np.maximum(0, X2)
        X4 = X3.dot(W2) + b2.T
        P = np.exp(X4) / np.sum(np.exp(X4), axis = 1, keepdims = True)
        Y = np.zeros_like(X4)
        ...
        grads['W1'] = X1.T.dot(d1.dot(W2.T) * d3) / N + reg * W1
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            X_batch = np.empty([batch_size, dim])
            
            y_batch = np.zeros([batch_size])
            

            for i in range(batch_size):
                rand = np.random.choice(np.arange(num_train))
                X_batch[i,:] = X[rand,:]
                y_batch[i] = y[rand]
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    for i in learning_rates:
        for j in regularization_strengths:
            for k in hidden_size:
                TLN = TwoLayerNet(input_size=3072, hidden_size=k, output_size=10)
                stats = TLN.train(X_train, y_train, X_val, y_val, num_iters=5000, batch_size=200, learning_rate=i, learning_rate_decay=0.95, reg=j, verbose=False)
                val = np.mean(y_val == TLN.predict(X_val))

                print("learning_rate:%f    reg:%f    hidden:%s     val:%s" %(i,j,k,val))
                print("")
    ...
    print("best_learning_rate:%f    best_reg:%f    best_hidden:%s     best_val:%s" %(best_learning_rate, best_reg, best_hidden, best_val))

...
def affine_forward(x, w, b):
    ...
    """
    N = x.shape[0]
    # Today's Learning : np.prod()
    D = np.int(np.prod(x.shape) / N)
...
def batchnorm_backward(dout, cache):
    ...
    dzdx = 1/cache[6]
    dzdu = -1 / cache[6]
    dzdv = -0.5 * (cache[6]**3) * cache[4]
    dvdu = -2/N * np.sum(cache[4], axis=0)
